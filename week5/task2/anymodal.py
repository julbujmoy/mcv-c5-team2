...
class MultiModalModel(nn.Module):
    ...
    def generate(self, input_data, max_new_tokens=100, **kwargs):
        ...
        if 'pad_token_id' not in kwargs:
            kwargs['pad_token_id'] = self.language_tokenizer.pad_token_id
            # If it's still None, fallback to eos_token_id
            if kwargs['pad_token_id'] is None:
                kwargs['pad_token_id'] = self.language_model.config.eos_token_id
