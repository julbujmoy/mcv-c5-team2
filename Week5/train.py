...
extra_train="/export/home/c5mcv02/diffusion/outputs_task4/"


# Define the dataset class
class FoodDataset(Dataset):
    def __init__(self, dataframe, processor, image_root,extra_train=False):
        self.processor = processor
        self.image_root = image_root

        combined_data = []

        # Load from CSV (original dataset)
        if dataframe is not None:
            filtered_df = self.data = dataframe[dataframe["Image_Name"].apply(lambda img: os.path.exists(os.path.join(image_root, str(img) + ".jpg")))].reset_index(drop=True)
            combined_data.append(filtered_df)

        # Load additional data from image filenames
        if extra_train:

            image_files = [f for f in os.listdir("/export/home/c5mcv02/diffusion/outputs_task4/") if f.lower().endswith(('.jpg', '.png'))]
            image_names = []
            captions = []
            ...
            for img_file in image_files:
                ...
                if parts[0].isdigit():
                    parts = parts[1:]
                caption = ' '.join(parts).strip().replace('  ', ' ')
                image_names.append("/export/home/c5mcv02/diffusion/outputs_task4/"+img_file)
            ...
            combined_data.append(extra_df)

        # Combine datasets (original + extra)
        self.data = pd.concat(combined_data, ignore_index=True) if combined_data else pd.DataFrame(columns=["Image_Name", "Title"])
...
config = wandb.config
train_dataset = FoodDataset(train_annotations, image_processor,train_img_folder,extra_train=True)
...
for epoch in range(num_epochs):
    ...
    if score > best_score:
        count=0
        best_score = score
        multimodal_model._save_model("/export/home/c5mcv02/laura_w5/AnyModal/Image_Captioning/DIFFUSIONfinal_model3b-rank2/")
    ...
    # multimodal_model._save_model("/export/home/c5mcv02/laura_w5/AnyModal/Image_Captioning/final_model/")

multimodal_model._save_model("/export/home/c5mcv02/laura_w5/AnyModal/Image_Captioning/DIFFUSIONlast_model3b-rank2/")
