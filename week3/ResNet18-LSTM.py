...
img_folder = "FoodImages/images/"  # Update the path accordingly
...
class CaptionModel(nn.Module):
    def __init__(self):
        super().__init__()
        self.resnet = ResNetModel.from_pretrained("microsoft/resnet-18").to(device)  # Usar ResNet18
        self.lstm = nn.LSTM(512, 512, num_layers=1, batch_first=True)  # Usar LSTM
        self.proj = nn.Linear(512, len(chars))
        self.embed = nn.Embedding(len(chars), 512)

    def forward(self, img):
        batch_size = img.shape[0]
        feat = self.resnet(img).pooler_output  # Extraer solo la salida correcta
        feat = feat.view(batch_size, 1, -1)  # Ajustar dimensiones a (batch, 1, 512)
        
        start = torch.full((batch_size, 1), char2idx['<SOS>'], dtype=torch.long, device=device)
        inp = self.embed(start)  # batch, 1, 512
        hidden = feat.permute(1, 0, 2)  # 1, batch, 512
        outputs = []
...
def train(model, dataloaders, criterion, optimizer, epochs=40):
    ...
    # Guardar el modelo
    torch.save(model.state_dict(), "recipe_model_resnet18_lstm.pth")
    print("Modelo guardado como recipe_model_resnet18_lstm.pth")

    # Subir el modelo a wandb
    wandb.save("recipe_model_resnet18_lstm.pth")
