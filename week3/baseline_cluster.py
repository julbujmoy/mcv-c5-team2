...
text_representation = "word"  # Options: "char", "word", "wordpiece"
TEXT_MAX_LEN = 201

# Initialize wandb
wandb.init(
    project="baseline",
    name="ResNet18-GRU-word",
    config={
        "epochs": 100,
        "batch_size": 32,
        "learning_rate": 0.001,
...
class CaptionModel(nn.Module):
    ...
    def forward(self, img):
        ...
        feat = feat.view(batch_size, 1, -1)  # (batch, 1, 512)
        # Convertir a forma (1, batch, 512) y replicar para num_layers = 5
        hidden = feat.permute(1, 0, 2).repeat(self.gru.num_layers, 1, 1)  # (5, batch, 512)
