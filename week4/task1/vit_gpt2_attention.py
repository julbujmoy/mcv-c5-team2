...
wandb.init(
    ...
    entity='laurasalort',
    name="vit_gpt2_attention",
    config={
        "epochs": 50,
        "batch_size": 32,
        "learning_rate": 1e-5,
        "model": "vit_gpt2_attention"
    }
)
config = wandb.config
...
class ImageCaptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        tokenized_caption = tokenizer(caption, return_tensors="pt", padding="max_length", truncation=True, max_length=50)
...
for epoch in range(num_epochs):
    ...
    total_loss = 0
    for images, captions, attention_masks in loop:
        images, captions, attention_masks = images.to(device), captions.to(device), attention_masks.to(device)
        
        optimizer.zero_grad()
        outputs = model(pixel_values=images, labels=captions, decoder_attention_mask=attention_masks)
        loss = outputs.loss
        loss.backward()
        optimizer.step()
    ...
    references = []
    with torch.no_grad():
        for images, captions, attention_masks in tqdm(dataloader_valid, desc=f"Epoch {epoch+1} Validation"):
            images, captions, attention_masks = images.to(device), captions.to(device), attention_masks.to(device)
            outputs = model(pixel_values=images, labels=captions, decoder_attention_mask=attention_masks)
            val_loss += outputs.loss.item()
            
            generated_ids = model.generate(pixel_values=images, max_length=50, num_beams=4, temperature=0.7)
    ...
    if avg_val_loss < best_val_loss:
        ...
        counter = 0
        torch.save(model.state_dict(), "vit_gpt2_attention.pth")
...
model.save_pretrained("vit_gpt2_attention")
tokenizer.save_pretrained("vit_gpt2_attention")
