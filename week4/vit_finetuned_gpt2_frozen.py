...
wandb.init(
    ...
    entity='laurasalort',
    name="vit_finetuned_gpt2_frozen",
    config={
        "epochs": 100,
        "batch_size": 32,
        "layers":1,
        "learning_rate": 1e-5,
        "model": "vit_finetuned_gpt2_frozen"
    }
)
config=wandb.config
...
# Configuración del optimizador solo para ViT
...
class ImageCaptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        caption = str(self.annotations.iloc[idx]["Title"])  # Asegurar que sea string
...
for epoch in range(num_epochs):
    ...
    with torch.no_grad():
        for images, captions in tqdm(dataloader_valid, desc=f"Epoch {epoch+1} Validation"):
            ...
            generated_ids = model.generate(pixel_values=images, max_length=16, num_beams=4)
    ...
    if avg_val_loss < best_val_loss:
        ...
        counter = 0
        torch.save(model.state_dict(), "vit_finetuned_gpt2_frozen.pth")
...
model.save_pretrained("vit_finetuned_gpt2_frozen")
tokenizer.save_pretrained("vit_finetuned_gpt2_frozen")
