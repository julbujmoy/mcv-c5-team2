...
wandb.init(
    ...
    entity='laurasalort',
    name="vit_gpt2_teacherF",
    config={
        "epochs": 100,
        "batch_size": 32,
        "learning_rate": 1e-5,
        "teacher_forcing_ratio": 0.5,  # Añadido teacher forcing
        "model": "vit_gpt2_teacherF"
    }
)
config = wandb.config
...
for epoch in range(num_epochs):
    ...
    for images, captions in loop:
        ...
        if torch.rand(1).item() < config.teacher_forcing_ratio:
            ...
            captions = tokenizer(captions, return_tensors="pt", padding="max_length", truncation=True, max_length=50).input_ids.to(device)
            outputs = model(pixel_values=images, labels=captions)
    ...
    if avg_val_loss < best_val_loss:
        ...
        counter = 0
        torch.save(model.state_dict(), "vit_gpt2_teacherF.pth")
...
model.save_pretrained("vit_gpt2_teacherF")
tokenizer.save_pretrained("vit_gpt2_teacherF")
wandb.finish()
